...
from tensorrt_llm import Module, ModuleList


class PansegformerHead(Module):
    """
    ...
    def _init_layers(self):
        ...
        self.reg_branches2 = _get_clones(
            reg_branch, self.num_dec_things)  # used in mask decoder
        self.cls_thing_branches = _get_clones(
            fc_cls, self.num_dec_things)  # used in mask decoder
        self.cls_stuff_branches = _get_clones(
            fc_cls_stuff, self.num_dec_stuff)  # used in mask deocder
    ...
    def forward(self, bev_embed):
        ...
        mlvl_feats = [torch.reshape(
            bev_embed, (bs, self.bev_h, self.bev_w, -1)).permute(0, 3, 1, 2)]
        ...
        args_tuple = [memory, memory_mask, memory_pos,
                      query, None, query_pos, hw_lvl]
    ...
    def loss_single_panoptic(self,
        ...
        for i in range(BS):
            ...
            for j in range(self.num_dec_things):
                ...
                tmp = self.reg_branches2[j](
                    query_things.reshape(t1 * t2, t3)).reshape(t1, t2, 4)
                if len(pos_ind) == 0:
                    tmp = tmp.sum(
                ...
                new_bbox_preds[j][i][:len(
                    pos_inds_mask_list[i])] = outputs_coord
    ...
    def forward_test(self,
        ...
        with torch.no_grad():
            ...
            divider_gt = (
                gt_lane_masks[0][0][gt_lane_labels[0][0] == 0].sum(0) > 0).int()
            crossing_gt = (
                gt_lane_masks[0][0][gt_lane_labels[0][0] == 1].sum(0) > 0).int()
            contour_gt = (
                gt_lane_masks[0][0][gt_lane_labels[0][0] == 2].sum(0) > 0).int()
    ...
    def get_bboxes(
        ...
        for img_id in range(len(img_metas)):
            ...
            id_unique = 1
            lane = torch.zeros(
                (self.num_things_classes, *mask_pred.shape[-2:]), device=mask_pred.device).to(torch.long)
            lane_score = torch.zeros(
                (self.num_things_classes, *mask_pred.shape[-2:]), device=mask_pred.device).to(mask_pred.dtype)
            ...
            file_name = img_metas[img_id]['pts_filename'].split(
                '/')[-1].split('.')[0]
